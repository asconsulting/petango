...
class Updater 
{
	...
	private function loadAll() {
		...
		foreach($this->objAllXml->XmlNode as $objNode) {
			if (trim((string)$objNode->adoptableSearch->ID) != '') {
				$arrAnimal = array();
	...
	private function loadAnimal($strAnimalId) {
		...
		$strAltered = strtolower(trim((string)$objAnimal->Altered));
		if ($strAltered == "yes" || $strAltered == "y") {$arrAnimal['altered'] = '1';}
		else {$arrAnimal['altered'] = '';}
